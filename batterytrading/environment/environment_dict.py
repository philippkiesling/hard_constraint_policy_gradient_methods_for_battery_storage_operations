...
class ContinousEnergyArbitrageEnvironment(core.Env):
    ...
    def step(self, action):
        ...
        if self.wandb_log:
            if self.eval_env:
                self.wandb_run.log({f"{self.prefix}EVAL_reward": rewards,
                                f"{self.prefix}EVAL_action": action,
                                f"{self.prefix}EVAL_price": price,
                                f"{self.prefix}EVAL_SOC": self.SOC,
                                f"{self.prefix}EVAL_action_valid": float(self.action_valid[-1]),
                                f"{self.prefix}EVAL_total_earnings": self.TOTAL_EARNINGS,
                                f"{self.prefix}EVAL_monthly_earnings": np.mean(self.ROLLING_EARNINGS[-672:]),
                                f"{self.prefix}EVAL_monthly_earnings (daily average)": np.mean(self.ROLLING_EARNINGS[-672:])*(24*4),
...
class ContinousEnergyArbitrage_Solar_Environment(core.Env):
    def __init__(self, max_charge=0.15,
                 total_storage_capacity=1,
                 initial_charge=0.0,
                 max_SOC=1,
...
class DiscreteContinousEnergyArbitrageEnvironment(ContinousEnergyArbitrage_Solar_Environment):
    def __init__(self, max_charge=0.15,
                 total_storage_capacity=1,
                 initial_charge=0.0,
                 max_SOC=1,
...
if __name__ == "__main__":
    env = ContinousEnergyArbitrage_Solar_Environment(data_root_path="../../")
